<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="生鲜类型" prop="shengxianleixing">
          <el-select :disabled="ro.shengxianleixing" v-model="ruleForm.shengxianleixing" placeholder="请选择生鲜类型">
            <el-option
                v-for="(item,index) in shengxianleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="生鲜类型" prop="shengxianleixing">
	      <el-input v-model="ruleForm.shengxianleixing"
                placeholder="生鲜类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="打捞日期" prop="dalaoriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.dalaoriqi" 
                type="date"
                :readonly="ro.dalaoriqi"
                placeholder="打捞日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.dalaoriqi" label="打捞日期" prop="dalaoriqi">
              <el-input v-model="ruleForm.dalaoriqi" 
                placeholder="打捞日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.shengxiantupian" label="生鲜图片" prop="shengxiantupian">
          <file-upload
          tip="点击上传生鲜图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.shengxiantupian?ruleForm.shengxiantupian:''"
          @change="shengxiantupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.shengxiantupian" label="生鲜图片" prop="shengxiantupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shengxiantupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="生鲜介绍" prop="shengxianjieshao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.shengxianjieshao" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shengxianjieshao" label="生鲜介绍" prop="shengxianjieshao">
                    <span v-html="ruleForm.shengxianjieshao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	shengxianleixing : false,
	baozhiqi : false,
	dalaoriqi : false,
	shengxianjieshao : false,
	shengxiantupian : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shengxianmingcheng = true;
          ...
          if(o=='shengxianleixing'){
            this.ruleForm.shengxianleixing = obj[o];
	    this.ro.shengxianleixing = true;
            continue;
          }
          if(o=='baozhiqi'){
	    ...
	    this.ro.baozhiqi = true;
          ...
          if(o=='dalaoriqi'){
            this.ruleForm.dalaoriqi = obj[o];
	    this.ro.dalaoriqi = true;
            continue;
          }
          if(o=='shengxianjieshao'){
            this.ruleForm.shengxianjieshao = obj[o];
	    this.ro.shengxianjieshao = true;
            continue;
          }
          if(o=='shengxiantupian'){
            this.ruleForm.shengxiantupian = obj[o];
	    this.ro.shengxiantupian = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.lianxidianhua!=''&&json.lianxidianhua){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shengxianfenlei/leixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shengxianleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.shengxianjieshao = this.ruleForm.shengxianjieshao.replace(reg,'../../../springbootcug86/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.shengxiantupian!=null) {
		this.ruleForm.shengxiantupian = this.ruleForm.shengxiantupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    shengxiantupianUploadChange(fileUrls) {
	this.ruleForm.shengxiantupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
