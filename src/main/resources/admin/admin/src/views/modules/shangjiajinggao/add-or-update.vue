<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="警告时间" prop="jinggaoshijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jinggaoshijian" label="警告时间" prop="jinggaoshijian">
              <el-input v-model="ruleForm.jinggaoshijian" 
                placeholder="警告时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="商店编号" prop="shangdianbianhao">
          <el-select :disabled="ro.shangdianbianhao" @change="shangdianbianhaoChange" v-model="ruleForm.shangdianbianhao" placeholder="请选择商店编号">
            <el-option
                v-for="(item,index) in shangdianbianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.shangdianbianhao" label="商店编号" prop="shangdianbianhao">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="警告内容" prop="jinggaoneirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.jinggaoneirong" label="警告内容" prop="jinggaoneirong">
                    <span v-html="ruleForm.jinggaoneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shangjia/shangdianbianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shangdianbianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    shangdianbianhaoChange () {
      this.$http({
        url: `follow/shangjia/shangdianbianhao?columnValue=`+ this.ruleForm.shangdianbianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.shangdianmingcheng){
                this.ruleForm.shangdianmingcheng = data.data.shangdianmingcheng
              }
        } else {
          this.$message.error(data.msg);
        }
	...
	this.ruleForm.jinggaoneirong = this.ruleForm.jinggaoneirong.replace(reg,'../../../springbootcug86/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shangjiajinggao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shangjiajinggaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
